...
To make your database up to date, you can run `yii migrate` in needed test folder, for example
if you are starting from `frontend` tests then you should run `yii migrate` in each suite folder `acceptance`, `functional`, `unit`
it will upgrade your database to the last state in accordance with migrations.
...
After all of those above have been done, you should be able to run your tests, for example to run `frontend` tests do:
...
You can also adjust your application suite configs and `_bootstrap.php` settings to use other urls and files, as it is can be done in `yii2-basic`.
